<?php include_once '../../konfiguracija.php'; 
provjeraOvlasti();

$greska=array();

if($_POST){
	include_once 'kontrole.php';
	
	
	if(count($greska)==0){
		unset($_POST["sifra"]);
		$izraz=$veza->prepare("insert into smjer (naziv, cijena, upisnina,brojsati) 
							values (:naziv, :cijena, :upisnina, :brojsati)");
		$izraz->execute($_POST);
		header("location: index.php");
	}

}

?>
<!doctype html>
<html class="no-js" lang="en" dir="ltr">
  <head>
    <?php include_once '../../include/head.php'; ?>
  </head>
  <body>
    <div class="grid-container">
    	<?php include_once '../../include/zaglavlje.php'; ?>
      	<?php include_once '../../include/izbornik.php'; ?>
      	<a href="builder.php?sifra=<?php echo $_POST["sifraDeck"]; ?>"><i style="color: red;" class="fas fa-chevron-circle-left fa-2x"></i></a>
      	<div class="grid-x grid-padding-x">
			<div class="large-4 large-offset-4 cell centered">
				<form class="log-in-form" action="" method="post">
				  <h4 class="text-center">Unesite podatke za novi smjer</h4>
				  
				  <?php if(!isset($greska["naziv"])): ?>
				  <label>Name
				    <input  type="text" id="naziv" name="naziv" placeholder=""
				    value="<?php echo isset($_POST["nazivDeck"]) ? $_POST["nazivDeck"] : ""; ?>">
				  </label>
				  <?php else: ?>
				   <label class="is-invalid-label">
				    Name
				    <input type="text"  id="naziv" name="naziv" class="is-invalid-input"  aria-invalid aria-describedby="uuid"
				    value="<?php echo isset($_POST["naziv"]) ? $_POST["naziv"] : ""; ?>" >
				    <span class="form-error is-visible" id="uuid"><?php echo $greska["naziv"]; ?></span>
				  </label>
				  <?php endif; ?>
				  
				  
				  
				  
				  <p><input type="submit" class="button expanded" value="Save"></input></p>
				</form>
				
			</div>
		</div>
		<?php include_once '../../include/podnozje.php'; ?>
		
      
    </div>

    <?php include_once '../../include/skripte.php'; ?>
    <script>
    <?php if(isset($greska["naziv"])):?>	
    		setTimeout(function(){ $("#naziv").focus(); },1000);	
    <?php elseif(isset($greska["cijena"])):?>	
	    		setTimeout(function(){ $("#cijena").focus(); },1000);	
	<?php elseif(isset($greska["upisnina"])):?>	
	    		setTimeout(function(){ $("#upisnina").focus(); },1000);	
	<?php elseif(isset($greska["brojsati"])):?>	
	    		setTimeout(function(){ $("#brojsati").focus(); },1000);	
	<?php endif; ?>
    </script>
  </body>
</html>
